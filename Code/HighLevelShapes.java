...
public class HighLevelShapes {

    ReadandWriteHighScore HiL = new ReadandWriteHighScore();
    // حيز ظهور الثعبان
    ...
    int bestScore = HiL.HiLReadHighScore();
    ...
    int ELxPos = random.nextInt(22);
    int ELyPos = 5 + random.nextInt(17);

    boolean isGameOver = false;

    public void HighLevelShapes(GraphicsContext Graphics) {
        ...
        int[] ELXPos = new int[26];
        int[] ELYPos = new int[26];
        for (int i = 0; i < 26; i++) {
            ELXPos[i] = i * 20 + 20;
            ELYPos[i] = i * 20 + 20;
        }
        ...
        HighLScreenInterFace SI = new HighLScreenInterFace(CurrentScore, fruitEaten, scoreReverseCounter, bestScore);
        SI.Show(Graphics);
...
ً            When Current Score > 1000
    ...
    public void GameOver(GraphicsContext Graphics) {
        ...
        HiL.HiLWriteHighScore(CurrentScore, bestScore);
    }
